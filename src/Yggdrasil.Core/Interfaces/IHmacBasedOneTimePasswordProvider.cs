...
public interface IHmacBasedOneTimePasswordProvider
{
    ...
    string CreateOneTimePassword(string secret, int movingFactor, int codeDigits = 6, bool addChecksum = true, int truncationOffset = 4);
    ...
    /// start offset.</param>
    /// <returns>A numeric String in base 10 that includes {@link codeDigits} digits plus the optional checksum digit if requested.</returns>
    string CreateOneTimePassword(byte[] secret, long movingFactor, int codeDigits = 6, bool addChecksum = true, int truncationOffset = 4);

    OneTimePasswordValidationResult ValidateOneTimePassword(string hotp, string secret, int movingFactor, int codeDigits = 6, bool addChecksum = true, int offset = 4);
}
